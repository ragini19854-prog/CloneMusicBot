...
async def play_live_stream(client, CallbackQuery, _):
    ...
    try:
        # ✅ FIX: Channel Play database function awaited
    ...
    try:
        # ✅ FIX: Track fetching awaited
        details, track_id = await YouTube.track(vidid, True)
    except:
        # ❌ REMOVED: os.system kill command (Bot Restart) removed.
    ...
    if not details["duration_min"]:
        try:
            # ✅ FIX: Stream function awaited (Database interaction inside)
            await stream(
                _,
                mystic,
                user_id,
