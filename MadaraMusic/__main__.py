...
from MadaraMusic.plugins.tools.clone import restart_bots
...
async def init():
    ...
    LOGGER("MadaraMusic.plugins").info("𝐀𝐥𝐥 𝐅𝐞𝐚𝐭𝐮𝐫𝐞𝐬 𝐋𝐨𝐚𝐝𝐞𝐝 𝐁𝐚𝐛𝐲🥳...")
    await userbot.start()
    await Lucky.start()
    try:
    ...
    await app.stop()
    await userbot.stop()
    LOGGER("MadaraMusic").info("𝗦𝗧𝗢𝗣 𝗠𝗨𝗦𝗜𝗖🎻 𝗕𝗢𝗧..")
